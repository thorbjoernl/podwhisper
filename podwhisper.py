...
def main():
    ...
    for i, x in enumerate(podcast.items, start = 1):
        logger.info("Processing item " + str(i) + "/" + str(len(podcast.items)) + ": " + x.title)

        audio_url = x.enclosure_url
        file_extension = pathlib.Path(audio_url).suffix

        # Paths for intermediary and output files.
        audio_path = os.path.join("tmp", sanitize_filename(podcast.title), hash(x.guid + x.title) + file_extension)
        text_path = os.path.join("out", sanitize_filename(podcast.title), sanitize_filename(x.title + ".txt"))
        segment_path = os.path.join("out", sanitize_filename(podcast.title), sanitize_filename(x.title + ".json"))

        already_transcribed = (os.path.exists(text_path) and os.path.exists(segment_path))
